...
| 2026-05-09 | v1.1.4 | @dev Neo | Phase 14.12 — *develop chunk 1.7 PATCH H4 preserve route protection MVP-LEAN-01 (Smith INFECTED HIGH). Edit bloco_interface/web/app.py GET / handler — adicionado session check + RedirectResponse('/login', 303) ANTES do serve SPA static (dual-protection defense-in-depth). Preserved MVP-LEAN-01 Task 2 AC-MVP-01 redirect logic. Pós SP04-AUTH-01 chunks 4 done (JWT cookie httpOnly), session check pode ser dropped. Verify: py_compile OK + grep RedirectResponse(/login) → 3 matches preserved. Próximo: handoff Smith *verify H4. |
| 2026-05-09 | v1.1.3 | @dev Neo | Phase 14.11 — *develop chunk 1.6 PATCH C2 brand-honest + NF1 (Smith INFECTED CRITICAL + NF1 LOW). 4 edits em SPA index.html: (1) meta description "BYOK · LGPD-aware" → "BYOK · Em formalização LGPD"; (2) login footer "Demo: análise simulada" → "Em construção: Sprint 04 finalização. Pipeline real habilitado chunks subsequentes. Sem dados pessoais até finalização compliance"; (3) sidebar footer "BYOK" → "BYOK · Em formalização LGPD"; (4) NF1 fix comment linha 13 "Zero CDN externo (sem fonts.googleapis/gstatic)" → "LGPD-COMPLIANT: dependências externas removidas, fontes self-hosted abaixo" (false positive scanner-prevention). Verify counts: LGPD-aware=0 ✅, Em formalização LGPD=2 ✅, Demo:=0 ✅, fonts.googleapis=0 ✅. Brand-honest temporário pattern AUTH-01 chunk 5 + LGPD-01 placeholder DPA precedent. Auto-revert planejado pós Eric advogado finaliza TOS canônico (~9.5h work) → reverter brand para "LGPD-aware" defensible. Próximo: handoff Smith *verify C2 + NF1 re-gate. |
| 2026-05-09 | v1.1.2 | @dev Neo | Phase 14.10 — *develop chunk 1.5 PATCH C1 self-host fonts (Smith INFECTED CRITICAL fix). Pattern REV-INT-02 (Sprint 02 commit 50a3b8b) REUSED — fontes self-hosted preserved em bloco_interface/web/static/fonts/ (7 arquivos: manrope-{400,500,600,700} + fraunces-500 + jetbrains-mono-{400,500}, ~117KB). SPA index.html linhas 10-12 (3 `<link>` Google Fonts CDN + 1 `<link>` stylesheet) substituídas por 7 inline @font-face declarations dentro existing `<style>` block + comment documentando zero CDN externo. Weights/fontes ausentes (Manrope 300/800, Fraunces variable axis, Frank Ruhl Libre 400/700) cobertos via fallback browser nativo — TD-SP04-15 LOW Sprint 06+ download adicionais. Verify: `grep "fonts.googleapis\|fonts.gstatic" bloco_interface/web/static/index.html` → 1 match em comment (intencional, NÃO funcional). LGPD NFR-LGPD-01 RESTORED. WAIVED-UI-CDN-01 evitado (não criado). Próximo: handoff Smith re-verify C1. IDS REUSE > ADAPT > CREATE Article IV-A honored — fontes existentes reutilizadas, weights ausentes documentados como tech debt. |
