...
| ~~**TD-PROCESS-01**~~ | Smith H6 reverify F-1 | ~~LOW~~ | ~~Process gap framework — ADR governance precisa hook obrigatório UX consultation~~ ✅ **RESOLVED 2026-05-10** — Morpheus Skill: Section "UX Expert Consultation Hook (SHOULD)" appended em `the_matrix/.claude/rules/adr-governance.md` (+62 linhas, local-only por política `.gitignore` linha 41 "framework lives local-only"). Trigger: ADRs com impacto visible-to-user surface (sidebar/navegação/IA/layout/forms/visual identity); Aria invoca Sati pré-flip; override documentado em ADR Histórico. | 2h (~25min real) | @lmas-master | framework | 2026-05-10 |
...
| ~~**TD-PROCESS-02**~~ | Neo gap analysis Smith chain | ~~LOW~~ | ~~Process gap framework — Smith adversarial review pré-merge MUST include CI/pytest status check~~ ✅ **RESOLVED 2026-05-10** — Morpheus Skill: Section "Smith FINAL Re-Gate CI Status Verification (MUST)" appended em `the_matrix/.claude/rules/quality-gate-enforcement.md` (+78 linhas, local-only). Trigger: Smith FINAL re-gate consolidado pré-merge OBRIGATORIAMENTE faz 1 de 3: (1) `gh pr checks {N}`; (2) pytest local rodado; (3) override documentado. Methodology distinction: spot-checks individuais OK sem CI; FINAL consolidado MUST com CI. Lesson learned Sprint 04 capturada como exemplo na rule. | 2h (~25min real) | @lmas-master | framework | 2026-05-10 |
